    ...
    return storageService.query(BOOKS_KEY)
        .then(books => {
            ...
            if (!books.length) {
                ...
                return storageService.postMany(BOOKS_KEY, books);
            }
            return books
        })
}
...
function saveReview(review, book) {
    return storageService.query(BOOKS_KEY)
        ...
        .then(books => {
            let updatedBook = books.find(item => item.id === book.id);
            ...
            storageService.put(BOOKS_KEY, updatedBook);
        })
}
...
function removeReview(bookId, idx) {
    return storageService.query(BOOKS_KEY)
        .then(books => { return books })
        .then(books => {
            let updatedBook = books.find(item => item.id === bookId);
...
function createBook(googleBook) {
    var gBook = googleBook.volumeInfo;
    var book = {
        "id": googleBook.id,
        "title": gBook.title,
        "subtitle": "mi est eros convallis auctor arcu dapibus himenaeos",
